# Emberlo website — v3 (gaming setups, rebranded)

A dependency-free static site (homepage, setup bundle pages, category
guides, a real-setups gallery, patch tracker) plus one serverless function
(`functions/api/match.js`) that does live setup matching using a real
Claude API call.
...
broadened from gaming-specific desks to general small-space UK home and
lifestyle decor — "Formwyn" and the .gg domain no longer fit once gaming
wasn't the throughline. The original Diablo 4 content is preserved in
`archive/d4-content/` rather than deleted, in case any of it is useful
again later. formwyn.gg remains registered but unused; the live domain is
now emberlo.co.uk.
...
- **Matching (`functions/api/match.js`, `lib/extract.js`) uses a real
  ...
  budget, and priorities, and turn it into structured tags.
...
guides.html, guides/       category guides — desks, chairs, monitor arms,
                           cable management, lighting, mini PCs
...
regenerates every setup and guide page from `data/setups.json` and
