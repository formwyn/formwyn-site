 ...
 * but I want it to look completely transformed") must be preserved as
...
const PRIORITY_TAGS = ['BUDGET_FIRST', 'MINIMAL_FOOTPRINT', 'STORAGE', 'AESTHETICS', 'COMFORT', 'MULTIPURPOSE', 'RENTAL_SAFE'];
const BUDGET_TAGS = ['ULTRA_BUDGET', 'BUDGET', 'MID', 'PREMIUM'];
const TENANCY_TAGS = ['RENTER', 'HOMEOWNER'];

const SYSTEM_PROMPT = `You convert someone's free-text description of their home/room decor needs into structured tags for a setup-matching system. You are reading intent, not just spotting keywords — colloquial phrasing counts as real signal, not just spec-sheet jargon.
...
- "priorities" is an array using ONLY these values: ${JSON.stringify(PRIORITY_TAGS)}. Use as many as genuinely apply. If the request is contradictory (e.g. wants it dirt cheap AND wants it to look completely transformed), include ALL the tags that reflect that contradiction — do not resolve it or pick just one, that happens downstream.
- "budget" is one of ${JSON.stringify(BUDGET_TAGS)} or null if not implied.
- "tenancy" is one of ${JSON.stringify(TENANCY_TAGS)} or null if not implied. RENTER = mentions renting, can't drill/mount things, landlord, temporary. HOMEOWNER = owns the place or explicitly says permanent fixtures/drilling is fine.
...
- AESTHETICS: wants it to look good, cosy, aesthetic, nice looking, Instagram-worthy, wants it to feel like home
- COMFORT: wants it to feel warm/cosy, somewhere to relax, comfortable
- MULTIPURPOSE: room needs to do more than one job — sleep and work, sleep and study, shared with someone else
- RENTAL_SAFE: renting, can't drill, landlord, don't want to lose my deposit, temporary, don't want to damage the walls

Worked examples:
Input: "I rent a tiny box room and want to spend as little as possible"
Output: {"roomSize": "BOX_ROOM", "priorities": ["BUDGET_FIRST", "MINIMAL_FOOTPRINT", "RENTAL_SAFE"], "budget": "ULTRA_BUDGET", "tenancy": "RENTER"}

Input: "I want it to feel cosy and put together but I genuinely can't spend much"
Output: {"roomSize": null, "priorities": ["AESTHETICS", "COMFORT", "BUDGET_FIRST"], "budget": null, "tenancy": null}
...
Input: "I own my place, want to go all out and actually fix up the whole room with proper storage"
Output: {"roomSize": "STANDARD_ROOM", "priorities": ["STORAGE", "AESTHETICS"], "budget": "PREMIUM", "tenancy": "HOMEOWNER"}`;
